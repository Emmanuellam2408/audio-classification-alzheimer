...
cwd = os.getcwd()
file_path_1 = os.path.join(cwd, 'au_cla_alz_master', 'top_5_mfcc.csv')
file_path_2 = os.path.join(cwd, 'au_cla_alz_master', 'text_features_labeled.csv')
...
model = svm.SVC(kernel='rbf', gamma=0.007, C=1, class_weight={'Control': 1, 'ProbableAD': 4})
...
X_1 = text_features
X_1 = pd.DataFrame(scaler.fit_transform(X_2), columns=X_2.columns)
...
X_2 = top_5
X_1 = pd.DataFrame(scaler.fit_transform(X_1), columns=X_2.columns)
...
predictions = np.array([pred_1, pred_2])  # Shape (3, n_samples)
...
X_1 = text_features_labeled.drop('dx', axis=1)
X_1 = pd.DataFrame(scaler.fit_transform(X_2), columns=X_2.columns)


y =  text_features_labeled.dx  # Target variable
